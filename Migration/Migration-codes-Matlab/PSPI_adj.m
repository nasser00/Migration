function M=PSPI_adj(data,dt,h,z0,deltaz,zmax,v,nrefv,fmin,fmax,padt,padx)
...
% This is a simple program for post stack PSPI migration
...
for l=1:izmax                     % loop over depth
    ...
    for mm=1:length(Kx)                        % loop over horizontal wavenumber
        kkz=(w(j)).^2/(refv(kk).^2)-Kx(mm).^2; % calculate depth shifting wavenumber
    Kz=sqrt(kkz);
