...
void nonstandard_header_init(char *name, int len, int ik)
/*< initialize non-segy standard trace headers >*/
{
    int namelen;

    if (ik < SF_NKEYS) 
	sf_error("%s: The non-standard header index ik is too low: %d < %d",
		 __FILE__,ik,SF_NKEYS);
    if (ik > SF_MAXKEYS) 
	sf_error("%s: The non-standard header index ik is too high: %d > %d",
		 __FILE__,ik,SF_MAXKEYS);
    namelen = strlen(name) + 1;
    segy_key[ik].name = sf_charalloc(namelen);
    strncpy((char*) segy_key[ik].name,name,namelen);
    segy_key[ik].size=len;
...
/* 	$Id: segy.c 9944 2013-02-21 16:26:54Z sfomel $	 */
